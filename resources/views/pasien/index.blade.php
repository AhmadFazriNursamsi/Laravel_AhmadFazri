...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight hetf2"><i class="bi bi-person-fill"></i>
            {{ __('PASIEN RUMAH SAKIT') }} <button type="button" class="btn btn-success btn-sm" id="buttonadd"> <i class="bi bi-person-fill"></i> Add Pasien</button>
        </h2>
    </x-slot>
                                                    {{-- HEADER --}}
    <div class="py-12">
                    ...
                    <div class="table-responsive">
                        <table id="RSTable" class="table text-start table-striped align-middle table-bordered table-hover mb-0">
                            <thead>
                                  ...
                                  <th class="align-center">Rumah Sakit</th>
                                ...
                                <dl class="row mb-0">
                                    ...
                                    <dd class="col-sm-8">: <input required type="text" name="alamat" id="alamat" class="form-group ">
                                        ...
                                        <dd class="col-sm-8">: <input required type="number" name="tlp" id="tlp" class="form-group ">
                                            ...
                                            <dd class="col-sm-8">: 
                                                <select name="id_rs" required class="form-control id_rs" id="id_id_rs">
                                                    <option selected >-- Select Users --</option>
                                                    @foreach ($db as $item)
                                                    <option id="id_rs" value="{{ $item->id }}">{{ $item->nama }}</option>
                                                        @endforeach
                                                </select>
                                            </dd>
    ...
    @section('script')
        <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-3-typeahead/4.0.1/bootstrap3-typeahead.min.js"></script>
            <script type="text/javascript">
                /////////////////////////////////////////////   GetData Table ///////////////////////////////////////////
                var url = "{{ asset('/api/pasien/getdata') }}";
                function getdata() { 
    ...
    $(document).ready(function(){
        ...
        $("#formm" ).submit(function(e) {
            var id = $('#id_name').val();
            var url= "{{ asset('/pasien/store') }}" ;
            if(id != '')
            var url= "{{ asset('/pasien/update') }}/" + id ;
            var form = $('#formm');
            e.preventDefault();
                $.ajax({
                    type: "POST",
    ...
    function showdetail(id) {
        ...
        $("#modaladd").modal('hide');
        $(".titlemodal").html("Pasien Rumah Sakit");
        $(".icoon").html('<i class="bi bi-card-list"></i>');
        $(".table-option").show();
    
        var url = "{{ asset('/pasien/detail') }}/" + id;
        // console.log(url);
            $.ajax({
            url: url,
            type: "GET",
            success: function(response) {
                ...
                if (data) {
                    $("#show_nama").html(data.nama_pasien);
                    $("#show_alamat").html(data.alamat);
                    // $("#show_logo").html(data.logo);
                    $.each(data.r_s, function (x, y) { 
                        // console.log(x,y);

                        $("#show_email").html(y.nama);
                     })
    ...
    function deleteshow(){
        ...
        idx = $('#deletebtn').attr('data-attid');
            ...
            if (result.isConfirmed) 
            {
                ...
                var url = "{{ asset('/pasien/delete') }}/" + idx;
                var form = $('#formm');
            // e.preventDefault();
                $.ajax({
                    type: "POST",
            ...
            console.log(url);
                ...
                success: function(response) {
                    ...
                    if (data) {
                        ...
                        $("#id_id_rs").append(' <option selected id="id_rs" value="'+y.id+'">'+y.nama+'</option>');
                     })
                        $("#tlp").val(data.tlp);
